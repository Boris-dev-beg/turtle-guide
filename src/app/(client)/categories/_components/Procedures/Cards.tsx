...
export const CardLoading = () => {
  return (
    <Card className="flex min-h-53 w-full flex-col overflow-hidden rounded-sm border-border bg-card p-5">
...
export const FoundedProceduces = ({
  ...
  return loading ? (
    ...
    <div className="flex items-center gap-2 text-brand-ink-muted">
      <span className="font-semibold text-brand-ink">{length}</span>
      <span>
        {length > 1 ? "procédures disponibles" : "procédure disponible"}
      </span>
    </div>
  );
};
