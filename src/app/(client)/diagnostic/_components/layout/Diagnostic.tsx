...
const CurrentQuestion = {
  ...
  answer_options: [
    ...
    },
    {
      ...
      description:
        "Je préfère obtenir plus d'informations avant de continuer.",
    },
  ],
};
...
export default function Diagnostic({ user }: { user: User }) {
  ...
  return (
    <>
      ...
      {/* Great to know */}
      <div className="turtle-alert-info flex-col items-start h-fit w-100 shadow-sm">
        ...
        <p className="text-muted-foreground text-sm leading-6">
          La déclaration doit être faite dans les 90 jours suivant la
          naissance pour éviter les démarches plus complexes.
        </p>
...
const FirstSide = ({ procedure }: { procedure: string }) => {
  return (
    <div className="h-full w-120 flex flex-col gap-4 py-4 px-2 border-r border-border bg-secondary/20 rounded-xl">
      {/* Back to home */}
      ...
      <div className="flex gap-3 items-start py-3 px-2 rounded-xl bg-background shadow-xs">
        ...
        <span className="w-full leading-4">
          ...
          <h2 className="font-bold line-clamp-2 leading-6">
            {procedure}
          </h2>
        </span>
      </div>
            ...
            <div className="flex gap-3 w-full" key={index}>
              <span className="flex flex-col items-center gap-2 text-muted-foreground justify-start">
                <span
                  ...
                  } w-5 h-5 ring-2 ring-background`}
                />

                {step.answer && (
                  <p className="h-6 border-l-2 border-border" />
                )}
              </span>
      ...
      {/* Help Box */}
      <div className="flex flex-col items-start gap-3 border border-border bg-background rounded-xl p-4 shadow-xs mt-auto">
  ...
  return (
    <div className="flex flex-col gap-5 p-5 rounded-xl border border-border bg-background w-full shadow-xs">
      ...
      <div className="flex flex-col gap-3 pb-2">
        ...
        <p className="text-muted-foreground flex flex-col gap-3 leading-6 max-w-3xl">
          {CurrentQuestion.description}

          <Link
            className="text-primary hover:underline flex gap-2 items-center ml-auto font-semibold transition-colors"
      ...
      {/* Actions */}
      <div className="flex w-full items-center justify-between border-t border-border pt-4 mt-1">
        <button className="btn btn-outline text-base rounded-lg">
          <ArrowLeft className="size-5" />
          Question précédente
        </button>

        <button className="btn btn-primary text-base rounded-lg px-5">
          Question suivante
